...
int main(int argc, char** argv)
{
    const char* keys =
        ...
        "{ backend    | ffmpeg | VideoCapture and VideoWriter backend }"
        "{ accel      | vaapi  | use GPU acceleration (Video Acceleration and OpenCL) }"
        "{ o output   |      | specify output save path, or 'null' to measure decoding only}"
    ...
    int accel = VIDEO_ACCELERATION_QSV;
    int camid = cmd.get<int>("c");
    bool gpu_mode = true; //cmd.get<bool>("gpu");

    ocl::setUseOpenCL(gpu_mode);

    VideoCapture capture(infile, backend);
    if (gpu_mode) {
        capture.set(CAP_PROP_HW_DEVICE, 0);
